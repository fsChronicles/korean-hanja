...
fetch('data.json')
    ...
    .then(data => {
        ...
        data.forEach(item => {
            // 본음, 장음 등은 모두 "첫말"로 변환
            if (item.구분 === '본음' || item.구분 === '장음') {
                item.구분 = '첫말';
            } else if (!item.구분.includes('끝말')) {
                // 끝말이 아닌 다른 값들도 첫말로 처리
                item.구분 = '첫말';
            }
            // 끝말은 그대로 유지 ("끝말", "끝말 - 1" 등)
        });

        // 구분(첫말/끝말) + 장단음별로 그룹화하여 번호 붙이기
        const grouped = {};
        data.forEach(item => {
            // 첫말/끝말을 기본값으로 추출 (번호 제거)
            const baseType = item.구분.includes('첫말') ? '첫말' : '끝말';
            // 장단음도 키에 포함 (장단음이 다르면 별개 항목)
            const key = `${item.한자}-${item.음}-${item.뜻}-${baseType}-${item.장단음}`;
            if (!grouped[key]) {
                grouped[key] = [];
            }
        ...
        // 각 그룹에서 2개 이상인 경우만 번호 붙이기 (역순)
        Object.values(grouped).forEach(group => {
            if (group.length > 1) {
                // 역순으로 번호 매기기: 배열 길이부터 1까지
                group.forEach((item, index) => {
                    // 이미 번호가 붙어있지 않은 경우에만 번호 추가
                    if (!item.구분.includes(' - ')) {
                        const reverseNumber = group.length - index;
                        item.구분 = `${item.구분} - ${reverseNumber}`;
                    }
                });
            }
            // 1개인 경우는 번호를 붙이지 않음 (이미 기본값)
        });
    ...
    .catch(error => {
        ...
        document.getElementById('tableBody').innerHTML =
            '<tr><td colspan="5" style="text-align:center;">데이터를 불러올 수 없습니다.</td></tr>';
    });
...
function displayData(data) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:40px;">검색 결과가 없습니다.</td></tr>';
    ...
    tbody.innerHTML = data.map(item => {
        // 훈음 형식: "뒤 후(後)"
        const huneum = `${item.뜻} ${item.음}(${item.한자})`;
        ...
        if (item.구분.includes('끝말')) {
            ...
            typeWeight = '600';
        } else if (item.구분.includes('첫말')) {
            typeColor = '#27ae60'; // 첫말은 녹색
            typeWeight = '600';
        }
